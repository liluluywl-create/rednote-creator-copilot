...
| 文档状态 | V1.0 需求与协议基线；不表示已经开发、上线或通过模型评测 |
...
产品帮助创作者完成三件同等重要的事：看清主页表达的问题；将自己的作品转成可发布图文；学习参考笔记的表达结构并应用到自己的素材。交付对象是报告、封面排版方案和可编辑文案，不是无边界聊天记录。

价值目标是减少从素材到可用草稿的操作与时间，提高表达质量和用户采纳。不得承诺爆款、涨粉、真实点击率或转化提升；未实测的数值一律标记为目标。

### 1.2 决策基线与范围

**纳入 T0：** 三个同等入口；长截图／多截图直接识别；疑似高反馈作品轻量确认；渐进式可选表单；成品三套方案横滑对比；原照片有限裁切与文字排版；仿写结果先行、拆解折叠；可选批注与手动编辑；3:4 图像区仿真预览；复制文案、保存封面；本机自动保存和首页历史记录。

**不纳入 T0：** 登录、账户、跨设备同步、用户可见创作者档案、长期风格画像、爬虫、笔记链接自动抓取、视频解析、复杂图像编辑、作品重绘、自动发布、趋势热榜检索、收藏体系、跨方案拼装、多代理编排、向量数据库、RAG、复杂后台管理。
...
| 喜欢同行表达但不想抄 | 首页→提交参考与自有素材→仿写卡片→按需展开拆解→预览复制 | 不读拆解也能拿到自己的文案 | 原文逻辑可追溯，自身经历不被编造 |
...
页面为：首页、主页诊断页、成品优化页、爆款拆解页，共用编辑面板与预览层。历史在首页下部展开，不新增账户页和复杂工作台。加载、确认、结果是页内状态，不增加路由步骤。

主页诊断：上传→识别骨架屏→标签轻确认→报告。成品优化：上传与渐进式表单→一次生成→三套比较→预览。爆款拆解：提交→先看仿写、拆解默认折叠→预览复制。任何模块不得把另一个模块设为前置条件。
...
| 参数 | V1.0 值 |
|---|---|
| 每任务图片数 | 1～6 张；仿写的参考图与自有图合计最多 6 张 |
...
| 文本输入 | 参考 ≤5,000 字；用户简报各字段及新主题合计 ≤1,000 字；空白输入规范化为 null |
...
| 输出 token 上限 | profile.inspect 1,500；post.inspect 1,500；profile.report 6,000；post.generate / viral.generate 10,000 |
...
| 近似表达检查阈值 | 归一化后连续相同片段 ≥18 字触发风险复核；不是原创判定标准 |
...
| `POST /api/v1/viral/generate` | `#/$defs/ViralGenerateRequest` | `#/$defs/ViralGenerateResponse` |
...
12. 各 Brief 字符串、新主题总计 ≤1,000 字；真实日历日期须有效。Schema maxLength 按 Unicode 字符计数，与高亮偏移采用的 UTF-16 单位区分。
...
**目标与前置条件。** 用户打开即能选择任务或继续草稿，无登录和档案填写。三入口同权重、同信息密度，不默认突出某个模块；历史位于入口下部。

#### 页面字段与回显

| 字段 / 组件 | 规则 |
|---|---|
| 任务入口 | 固定三个：主页诊断、成品图文优化、爆款拆解；各一句输入与输出说明 |
| 历史记录 kind | `profile / post / viral`，以图标和文字区分 |
...
**验收。** 三入口都能从首页直接到达；保存和恢复文字、选择状态一致；容量满不删旧稿；缓存损坏仅影响对应项；历史区不出现登录要求。
...
### 4.4 模块三：爆款逆向拆解与仿写（Viral Reverse-Engineering）
...
“仿写先给结果”指页面阅读顺序：先文案、后折叠解释，不要求网络上先流出文案再返回结构。本版仅展示经过完整校验的结果，不把未闭合JSON或推理过程推到UI。
...
| INSUFFICIENT_REFERENCE | 缺少可提取的参考文字或结构 | 保留自己的素材，补充参考 |
...
| SIMILARITY_UNRESOLVED | “与参考表达仍过于接近” | 保留用户输入，不输出伪“原创通过”标签 |
...
每个任务模板由六部分组成：任务与目标；可信用户事实；待分析图片／参考素材；字段和证据要求；少量正反例；对应data JSON Schema。少样本例必须自建或合法使用，包含输入与合格的结构化输出；不可用未经确认的同行经历作事实模板。
...
首批计划建立60个独立场景，三个模块各20个；每模块10个开发样例、10个锁定测试样例。开发样例用于Prompt迭代；锁定集30个样例每个重复3次，形成90次调用以观察随机性。**这些是拟建规模，不是已完成的调研或评测。** 重复调用不是90个独立样本，报告必须同时写独立样本数、调用次数和分母。

按创作者／作品／参考来源分组拆分，避免同一作品的近似截图同时出现在开发与锁定集。覆盖：完整与缺失参数、清晰与低清图、多风格、重复截图、参考仅封面、纯文字仿写、长文本、Emoji偏移、提示注入、数字幻觉和参考相近表达。网络、限额、缓存故障属于独立工程异常测试，不混入自然模型质量样本。
...
| 高亮准确率 | 正确对应参考原文的可见高亮数 ÷ 展示的高亮数 | 100%由应用匹配保证；无法匹配的撤去并统计缺失率 |
